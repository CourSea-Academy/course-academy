...
export const GlowingStarsBackgroundCard = ({ className, children }) => {
  ...
  return (
    <div
      ...
      className={cn(
        "bg-gradient-to-r from-primary to-primary p-6 lg:p-4 max-w-full min-h-[10rem] h-full w-full rounded-md ",
        className
      )}
    >
      <div className="flex justify-center items-center">
        <Illustration mouseEnter={mouseEnter} />
        <div className="px-2 absolute">{children}</div>
      </div>
    </div>
  );
};
...
export const GlowingStarsTitle = ({ className, children }) => {
  return (
    <h2
      className={cn(
        "relative font-bold text-xl lg:text-2xl z-[100] text-[#eaeaea]",
        className
      )}
    >
      {children}
...
export const Illustration = ({ mouseEnter }) => {
  ...
  return (
    <div
      className="min-h-[9rem] p-1 w-full"
      style={{
        display: "grid",
